...
static bool dldiPatchLoader (bool useSlot2) {
...
int runNds (const void* loader, u32 loaderSize, u32 cluster, bool initDisc, bool dldiPatchNds, int argc, const char** argv, bool isSlot2) {
	...
	*argData = argTempVal;
	...
	if(dldiPatchNds) {
		...
		if (!dldiPatchLoader (isSlot2))return 3;
	}
...
int runNdsFile (const char* filename, int argc, const char** argv, bool useSlot2)  {
	...
	/*#ifndef _NO_BOOTSTUB_
	installBootStub((argv[0][0]=='s' && argv[0][1]=='d'));
	#endif*/

	return runNds (load_bin, load_bin_size, st.st_ino, true, (memcmp(io_dldi_data->friendlyName, "Default", 7) != 0), argc, argv, useSlot2);
}
